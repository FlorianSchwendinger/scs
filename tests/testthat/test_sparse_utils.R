...
test_that("make_csc_sym_matrix works on plain matrices", {
  ms  <- as.matrix(s)
  cm  <- make_csc_symm_matrix.matrix(ms)
...
test_that("make_csc_sym_matrix works on simple triplet matrices", {
  ts  <- as(s, "dgTMatrix")
  sl  <- slam::simple_triplet_matrix(i = ts@i, j = ts@j, v = ts@x, nrow = 7, ncol = 7)

  tm  <- make_csc_symm_matrix.simple_triplet_matrix(sl)
